(() => {
  ...
  const VERSION = '0.3.6';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.6.json';
  ...
  const LATE_MISS_RATE = 0.10;
  const ROD_LENGTHS = [3.6, 4.5, 6.3, 7.2];

  const FLOAT_IMAGE_KEY = 'ui:float';
  const ROD_SOURCE = { handleX: 240, handleY: 220, tipX: 2460, tipY: 220 };
  const ROD_TEXTURES = {
    '3.6': { key: 'rod:3.6', image: './assets/rods/rod_3_6m.png', sections: 4, heightRatio: 0.43, widthRatio: 1.03, settleAngle: -2.08, backAngle: -1.42, forwardAngle: -2.52 },
    '4.5': { key: 'rod:4.5', image: './assets/rods/rod_4_5m.png', sections: 5, heightRatio: 0.50, widthRatio: 1.14, settleAngle: -2.06, backAngle: -1.41, forwardAngle: -2.54 },
    '6.3': { key: 'rod:6.3', image: './assets/rods/rod_6_3m.png', sections: 6, heightRatio: 0.60, widthRatio: 1.32, settleAngle: -2.03, backAngle: -1.39, forwardAngle: -2.57 },
    '7.2': { key: 'rod:7.2', image: './assets/rods/rod_7_2m.png', sections: 7, heightRatio: 0.68, widthRatio: 1.46, settleAngle: -2.00, backAngle: -1.37, forwardAngle: -2.60 }
  };
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.6 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function defaultStateLabel(state) {
    switch(state){
      ...
      case GameState.FIGHT: return `中鱼 · ${hookedFish ? hookedFish.species : ''}`;
  ...
  function populateBaitGrid() {
    ...
    ui['bait-grid'].innerHTML = entries.map(([key,bait]) => {
      ...
      const firstLayer = bait.probabilities.bottom;
      const targets = Object.entries(firstLayer).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([k]) => FISH_PROFILES[k].name).join('、');
      return `<button type="button" class="bait-option ${selected}" data-bait="${key}"><strong>${bait.name}</strong><span>目标：${targets}</span><span>${bait.description}</span><em>选择</em></button>`;
    }).join('');
  ...
  function startFight(hookElapsed) {
    ...
    updateFightUI();
    toast(`${hookedFish.species}中钩！大鱼越重，冲刺和张力越强。`, 2000);
  }
  ...
  function moveView(direction) {
    ...
    const step = 2.2;
    if (direction === 'left') viewOffsetX = clamp(viewOffsetX - step, -14, 14);
    if (direction === 'right') viewOffsetX = clamp(viewOffsetX + step, -14, 14);
    if (direction === 'forward') viewOffsetY = clamp(viewOffsetY - step, -8, 8);
    if (direction === 'back') viewOffsetY = clamp(viewOffsetY + step, -8, 8);
  }
  ...
  function castMotion(zone = castZone) {
    if (zone === 'near') return { lower: 0.12, lift: 0.20, sendDrop: 0.05, backswing: 7, arc: 0.070, bendDown: 0.020, bendUp: -0.018, durationScale: 0.90 };
    if (zone === 'far') return { lower: 0.30, lift: 0.38, sendDrop: 0.12, backswing: 24, arc: 0.145, bendDown: 0.065, bendUp: -0.060, durationScale: 1.08 };
    return { lower: 0.22, lift: 0.30, sendDrop: 0.09, backswing: 14, arc: 0.105, bendDown: 0.045, bendUp: -0.038, durationScale: 1.00 };
  }
  function virtualLeftHand() {
    return { x: canvasCssWidth * 0.40, y: canvasCssHeight * 0.750 };
  }
  function heldRigPose(now = performance.now()) {
    ...
    const sway = Math.sin(now / 260) * 2.2;
    const back = motion.backswing * charge;
    const float = { x: hand.x - back + sway, y: hand.y + 35 + charge * 8 };
    const sinker = { x: float.x + 2, y: float.y + 55 };
    const bait = { x: sinker.x - 1, y: sinker.y + 34 };
    return { hand, float, sinker, bait };
  }
  function rodAngle(now) {
    ...
    if (gameState === GameState.CHARGING) {
      return settle + motion.lower * clamp(castPower / 100, 0, 1);
    }
    if (gameState === GameState.FLIGHT || gameState === GameState.FLOAT_SETTLE) {
      const t = castPoseT;
      const charged = settle + motion.lower;
      const lifted = settle - motion.lift;
      const sent = settle + motion.sendDrop;
      if (t < 0.44) return lerp(charged, lifted, 1 - Math.pow(1 - t / 0.44, 2));
      if (t < 0.82) return lerp(lifted, sent, (t - 0.44) / 0.38);
      return lerp(sent, settle, (t - 0.82) / 0.18);
    }
    if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) return settle - 0.08 + Math.sin(now / 130) * 0.018;
    if (gameState === GameState.WAIT_BITE || gameState === GameState.BITE) return settle - 0.02;
    return settle;
  }
  function rodBend(now) {
    ...
    if (gameState === GameState.FLIGHT) {
      const t = castPoseT;
      if (t < 0.45) return lerp(motion.bendDown, motion.bendUp, t / 0.45);
      return lerp(motion.bendUp, 0, (t - 0.45) / 0.55);
    }
    return 0;
  }
  ...
  function computeRodPieces(now) {
    const cfg = rodTextureConfig();
    const handle = { x: canvasCssWidth * 0.50, y: canvasCssHeight * 0.945 };
    const totalLength = rodDisplayLength();
    const angle = rodAngle(now);
    const bend = rodBend(now);
    const boundaries = rodSourceBoundaries(cfg.sections);
    const sourceLens = [];
    sourceLens.push(boundaries[1] - ROD_SOURCE.handleX);
    for (let i=1;i<cfg.sections;i++) sourceLens.push(boundaries[i+1]-boundaries[i]);
    const sourceTotal = sourceLens.reduce((a,b)=>a+b,0);
    let current = { ...handle };
    const pieces = [];
    sourceLens.forEach((sourceLen,i) => {
      const localT = cfg.sections === 1 ? 0 : i/(cfg.sections-1);
      const pieceAngle = angle + bend * Math.pow(localT, 1.35);
      const length = totalLength * sourceLen/sourceTotal;
      const next = { x: current.x + Math.cos(pieceAngle)*length, y: current.y + Math.sin(pieceAngle)*length };
      pieces.push({ index:i, start:{...current}, end:next, angle:pieceAngle, length, sourceStart:i===0?0:boundaries[i], sourceEnd:boundaries[i+1] });
      current = next;
    });
  ...
  function castTarget(distance) {
    const ratio = clamp(distance / 14.4, 0.10, 1);
    return { x: canvasCssWidth * lerp(0.47, 0.72, ratio), y: lerp(getWaterY()+canvasCssHeight*0.12, getWaterY()-canvasCssHeight*0.012, ratio) };
  }
  function drawBackground() {
    ...
    const ox = (cw - dw) / 2 - viewOffsetX * 5;
    const oy = (ch - dh) / 2 - viewOffsetY * 4;
  ...
  function drawFloatSprite(x,y,rotation,scale=1) {
    const img=images[FLOAT_IMAGE_KEY], drawW=29*scale, drawH=96*scale;
    ctx.save(); ctx.translate(x,y); ctx.rotate(rotation);
    if(img) ctx.drawImage(img,-drawW/2,-drawH*.48,drawW,drawH);
    ctx.restore();
    return {top:rotatePoint(x,y,0,-drawH*.43,rotation),base:rotatePoint(x,y,0,drawH*.44,rotation)};
  }
  function drawSinker(x,y,rotation=0,scale=1) {
    ctx.save();ctx.translate(x,y);ctx.rotate(rotation);
    ctx.beginPath();ctx.ellipse(0,0,3.2*scale,7*scale,0,0,Math.PI*2);
    ctx.fillStyle='#595a55';ctx.fill();ctx.strokeStyle='#c5c3b7';ctx.lineWidth=.8;ctx.stroke();ctx.restore();
  }
  function drawBaitSprite(x,y,scale=1) {
    ctx.beginPath();ctx.arc(x,y,4.5*scale,0,Math.PI*2);ctx.fillStyle='#f2d188';ctx.fill();ctx.strokeStyle='#9c7440';ctx.lineWidth=1;ctx.stroke();
  }
  function drawVirtualLeftHand(open=false) {
    const hand=virtualLeftHand();
    ctx.save();ctx.translate(hand.x,hand.y);
    ctx.fillStyle='rgba(224,177,137,.88)';ctx.strokeStyle='rgba(91,63,47,.75)';ctx.lineWidth=1;
    ctx.beginPath();ctx.ellipse(0,0,12,9,-.15,0,Math.PI*2);ctx.fill();ctx.stroke();
    for(let i=0;i<3;i++){ctx.beginPath();ctx.roundRect(5+i*3,-8-i,3,open?12:8,2);ctx.fill();}
    ctx.restore();
  }
  function rigFlightProgress(t) {
    return { baitP: clamp(t/.82,0,1), sinkerP: clamp((t-.025)/.84,0,1), floatP: clamp((t-.075)/.925,0,1) };
  }
  function lowArc(start,target,p,height) {
    const e=1-Math.pow(1-p,2);
    return {x:lerp(start.x,target.x,e),y:lerp(start.y,target.y,e)-Math.sin(Math.PI*p)*height};
  }
  function drawRigLines(tip,floatPose,sinker,bait,hand=null) {
    ctx.strokeStyle='rgba(245,248,246,.88)';ctx.lineWidth=1.05;
    ctx.beginPath();ctx.moveTo(tip.x,tip.y);
    if(hand){ctx.lineTo(hand.x,hand.y);ctx.lineTo(floatPose.top.x,floatPose.top.y);}else ctx.lineTo(floatPose.top.x,floatPose.top.y);
    ctx.stroke();
    ctx.strokeStyle='rgba(240,245,242,.72)';ctx.lineWidth=.9;
    ctx.beginPath();ctx.moveTo(floatPose.base.x,floatPose.base.y);ctx.lineTo(sinker.x,sinker.y);ctx.lineTo(bait.x,bait.y);ctx.stroke();
  }
  function drawSplash(x,y,strength=1) {
    ctx.strokeStyle=`rgba(255,255,255,${.28*strength})`;ctx.lineWidth=1;
    ctx.beginPath();ctx.ellipse(x,y,12*strength,4*strength,0,0,Math.PI*2);ctx.stroke();
  }
  function drawFloatAndLine(now) {
    const tip=lineAnchor;
    if(gameState===GameState.READY||gameState===GameState.CHARGING){
      const held=heldRigPose(now);
      drawVirtualLeftHand(false);
      const fp=drawFloatSprite(held.float.x,held.float.y,.02,0.76);
      drawSinker(held.sinker.x,held.sinker.y,0,.9);drawBaitSprite(held.bait.x,held.bait.y,.9);
      drawRigLines(tip,fp,held.sinker,held.bait,held.hand);
      floatVisible=true;return;
    }
    if(gameState===GameState.FLIGHT&&flightRig){
      const t=clamp((now-castFlightStart)/castFlightDuration,0,1);castPoseT=t;
      drawVirtualLeftHand(true);
      const motion=castMotion(castZone),height=canvasCssHeight*motion.arc;
      const progress=rigFlightProgress(t), baitP=progress.baitP, sinkP=progress.sinkerP, floatP=progress.floatP;
      const bait=lowArc(flightRig.baitStart,flightRig.baitTarget,baitP,height*.86);
      const sinker=lowArc(flightRig.sinkerStart,flightRig.sinkerTarget,sinkP,height*.94);
      const flo=lowArc(flightRig.floatStart,flightRig.floatTarget,floatP,height);
      const rotation=lerp(-.42,0,floatP);
      const fp=drawFloatSprite(flo.x,flo.y,rotation,.78);
      drawSinker(sinker.x,sinker.y,rotation*.5,.9);drawBaitSprite(bait.x,bait.y,.9);
      drawRigLines(tip,fp,sinker,bait,null);
      if(baitP>=1)drawSplash(bait.x,bait.y,.8);
      if(t>=1){
    ...
    if(gameState===GameState.FLOAT_SETTLE){
      const t=clamp((now-rigSettleStart)/520,0,1);castPoseT=lerp(1,.82,t);
      const flo={x:floatPosition.x,y:lerp(floatPosition.y-18,floatPosition.y,t)};
      const fp=drawFloatSprite(flo.x,flo.y,lerp(.18,0,t),.78);
      const sinker={x:floatPosition.x+1,y:floatPosition.y+lerp(8,42,t)};
      const bait={x:floatPosition.x+2,y:floatPosition.y+lerp(10,72,t)};
      drawSinker(sinker.x,sinker.y,0,.9);drawBaitSprite(bait.x,bait.y,.9);drawRigLines(tip,fp,sinker,bait,null);drawSplash(floatPosition.x,floatPosition.y,1-t*.5);
      if(t>=1){setState(GameState.WAIT_BITE,'等待鱼口');ui['distance-label'].textContent=`${castDistance.toFixed(1)} m`;scheduleBite(rnd(2200,5600));}
      return;
    }
    if(floatVisible||floatLanded||gameState===GameState.WAIT_BITE||gameState===GameState.BITE||gameState===GameState.FIGHT||gameState===GameState.NET_READY){
      castPoseT=0;let bob=0;
      if(gameState===GameState.WAIT_BITE)bob=Math.sin(now/320)*1.6;
      if(gameState===GameState.BITE){const p=(performance.now()-biteStartAt)/Math.max(1,biteWindowEnd);bob=Math.sin(p*10)*2.2+(bitePattern==='黑漂'?10*p:bitePattern==='顶漂'?-6*p:4*Math.sin(p*15));}
      if(gameState===GameState.FIGHT||gameState===GameState.NET_READY)bob=Math.sin(now/120)*2.8;
      const flo={x:floatPosition.x,y:floatPosition.y+bob+(getDepthState().marks===5?-4:0)};
      const fp=drawFloatSprite(flo.x,flo.y,.01,.70);
      const sinker={x:floatPosition.x+1,y:floatPosition.y+45};const bait={x:floatPosition.x+2,y:floatPosition.y+76};
      drawSinker(sinker.x,sinker.y,0,.75);drawBaitSprite(bait.x,bait.y,.75);drawRigLines(tip,fp,sinker,bait,null);
      ctx.beginPath();ctx.arc(floatPosition.x,floatPosition.y+18,16+Math.sin(now/350)*1.5,0,Math.PI*2);ctx.strokeStyle='rgba(255,255,255,.18)';ctx.stroke();
    }
  }
  function drawMovementHint() {
    ...
    ctx.fillText('初始线组已挂好；按住“抛竿”蓄力。', x+rw/2, y+36);
  }
  function render(now) {
    syncCanvasSize();
  ...
  function releaseCast() {
    ...
    flightRig={
      hand:{...held.hand},floatStart:{...held.float},sinkerStart:{...held.sinker},baitStart:{...held.bait},
      floatTarget:{...target},sinkerTarget:{x:target.x+1,y:target.y+10},baitTarget:{x:target.x+3,y:target.y+5}
    };
  ...
  async function preloadImages() {
    const all = [...SPOTS.map(s => ({ key: s.id, src: s.image })), ...Object.entries(FISH_PROFILES).map(([key,val]) => ({ key: `fish:${key}`, src: val.image })), ...Object.values(ROD_TEXTURES).map(rod => ({ key: rod.key, src: rod.image })), { key: FLOAT_IMAGE_KEY, src: './assets/ui/float_real.png' }];
    let loaded = 0;
  ...
  async function boot() {
    ...
    await loadConfig();
    setLoading('正在加载钓场、鱼图、鱼竿与浮漂贴图…', 12);
    await preloadImages();
