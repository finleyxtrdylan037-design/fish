(() => {
  ...
  const PROBABILITY_FILE = "./bait_depth_fish_probability_v0.2.3.json";
  ...
  function constrainCamera(dt) {
    ...
    if ([GameState.READY, GameState.RIG_SETUP].includes(gameState)) {
      ...
      r.y = 0;
      r.normalize();
      if (moveDirection.forward) camera.position.addInPlace(f.scale(s));
      if (moveDirection.back) camera.position.addInPlace(f.scale(-s));
      if (moveDirection.left) camera.position.addInPlace(r.scale(-s));
      if (moveDirection.right) camera.position.addInPlace(r.scale(s));
    }
    camera.position.x = clamp(camera.position.x, -12, 12);
    camera.position.z = clamp(camera.position.z, -6, -0.75);
  ...
  function interactions() {
    ...
    document.querySelectorAll("[data-close-dialog]").forEach(
      (b) =>
        (b.onclick = () => {
          closeDialog(document.getElementById(b.dataset.closeDialog));
