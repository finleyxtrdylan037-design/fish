(() => {
  ...
  const VERSION = '0.3.1';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.1.json';
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.1 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function getWaterY() {
    return canvas.height * currentSpot().waterline;
  }
  function castTarget(distance) {
    const ratio = clamp(distance / maxCastDistance(), 0.18, 1);
    const x = canvas.width * lerp(0.48, 0.77, ratio);
    const y = getWaterY() + 6;
    return { x, y };
  }
  ...
  function drawFloatAndLine(now) {
    ...
    if (gameState === GameState.FLIGHT && flightRig) {
      const t = clamp((now - castFlightStart) / castFlightDuration, 0, 1);
      const p1x = lerp(flightRig.from.x, flightRig.to.x, t);
      const p1y = lerp(flightRig.from.y, flightRig.to.y, t) - Math.sin(Math.PI*t) * 140;
      rigX = p1x; rigY = p1y;
      if (t >= 1) {
    ...
    ctx.strokeStyle = 'rgba(240,245,246,0.78)';
    ctx.lineWidth = 1.2;
    ctx.stroke();
    // bait
    ctx.beginPath();
    ...
    ctx.fill();
    // float only once near water/flight
    const marks = ['#ff5858','#ffe172','#1a1a1a','#70d779','#ff5858','#ffe172'];
    const topOffset = getDepthState().marks === 5 ? -40 : -34;
    marks.forEach((color, i) => {
      ctx.fillStyle = color;
      ctx.fillRect(rigX - 3.5, rigY + topOffset + i*6, 7, 6);
    });
    ctx.beginPath();
    ctx.ellipse(rigX, rigY + 2, 7.5, 18, 0, 0, Math.PI*2);
    ...
    ctx.stroke();
    // landing ring marker
    if (gameState === GameState.WAIT_BITE || gameState === GameState.BITE || gameState === GameState.FIGHT || gameState === GameState.NET_READY) {
      ctx.beginPath();
  ...
  function drawMovementHint() {
    const w = canvas.width, h = canvas.height;
    ctx.fillStyle = 'rgba(10,18,14,0.44)';
    ctx.beginPath();
    const x = w*0.42, y = h*0.33, rw = Math.min(420, w*0.56), rh = 64;
    const r = 28;
    ...
    ctx.font = `${Math.max(18, canvas.width*0.018)}px sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillText(`当前距离水边约1米；按住“抛竿”蓄力。`, x+rw/2, y+39);
  }
  function render(now) {
