(() => {
  ...
  const VERSION = '0.3.4';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.4.json';
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.4 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  const FISH_PROFILES = {
    crucian_carp: { name: '鲫鱼', minKg: 0.08, maxKg: 1.8, minCm: 12, maxCm: 34, sizeBias: 2.1, power: 0.9, note: '底层觅食，警惕性较高。', bites: ['顿口','顶漂','下顿后回升'], image: './assets/fish/crucian_carp.png' },
    common_carp: { name: '鲤鱼', minKg: 0.3, maxKg: 12, minCm: 22, maxCm: 92, sizeBias: 1.45, power: 1.6, note: '吃口稳，发力厚重。', bites: ['顿口','黑漂','送漂'], image: './assets/fish/common_carp.png' },
    wuchang_bream: { name: '鳊鱼', minKg: 0.2, maxKg: 3.5, minCm: 18, maxCm: 50, sizeBias: 1.9, power: 1.1, note: '中下层巡游，吃口偏轻。', bites: ['顿口','送漂'], image: './assets/fish/wuchang_bream.png' },
    silver_carp: { name: '鲢鱼', minKg: 1, maxKg: 10, minCm: 38, maxCm: 95, sizeBias: 1.35, power: 1.7, note: '中上层活动，抖动频繁。', bites: ['黑漂','下顿后回升'], image: './assets/fish/silver_carp.png' },
    bighead_carp: { name: '鳙鱼', minKg: 1.2, maxKg: 15, minCm: 40, maxCm: 105, sizeBias: 1.25, power: 1.85, note: '冲刺强，头重身宽。', bites: ['黑漂','顿口'], image: './assets/fish/bighead_carp.png' },
    grass_carp: { name: '草鱼', minKg: 0.8, maxKg: 16, minCm: 35, maxCm: 115, sizeBias: 1.35, power: 1.95, note: '中下层大体型，冲刺明显。', bites: ['顿口','黑漂','送漂'], image: './assets/fish/grass_carp.png' },
    tilapia: { name: '罗非鱼', minKg: 0.12, maxKg: 4.2, minCm: 14, maxCm: 48, sizeBias: 1.8, power: 1.0, note: '罗非吃口快，连口率高。', bites: ['顿口','顶漂'], image: './assets/fish/tilapia.png' },
    topmouth_culter: { name: '翘嘴', minKg: 0.15, maxKg: 6.2, minCm: 18, maxCm: 75, sizeBias: 1.55, power: 1.4, note: '中上层掠食，动作干脆。', bites: ['送漂','顿口','黑漂'], image: './assets/fish/topmouth_culter.png' },
    black_carp: { name: '青鱼', minKg: 1.5, maxKg: 20, minCm: 42, maxCm: 120, sizeBias: 1.15, power: 2.2, note: '底层大物，冲刺与张力都很强。', bites: ['黑漂','顿口'], image: './assets/fish/black_carp.png' },
  };
  ...
  function updateButtons() {
    ...
    ui['primary-action'].disabled = gameState === GameState.RESULT || gameState === GameState.FIGHT || gameState === GameState.FLIGHT;
  ...
  function triggerBite() {
    ...
    setTimeout(() => {
      if (gameState === GameState.BITE) missBite('提竿过晚，鱼已吐钩。');
    }, biteWindowEnd + 220);
  }
  function attemptHook() {
    ...
    if (elapsed > biteWindowEnd) { missBite('提竿过晚，鱼已吐钩。'); return; }
    startFight(elapsed);
  }
  ...
  function hangingRigLength(length = rodLength) {
    const cfg = rodTextureConfig(length);
    return clamp(rodDisplayLength(length) / cfg.sections, 44, 120);
  }
  function rodAngle(now) {
    const cfg = rodTextureConfig();
    if (gameState === GameState.CHARGING) {
      const p = clamp(castPower / 100, 0, 1);
      return lerp(cfg.settleAngle + 0.05, cfg.backAngle, p);
    }
    if (gameState === GameState.FLIGHT) {
      const t = castPoseT;
      if (t < 0.32) return lerp(cfg.backAngle, cfg.forwardAngle, t / 0.32);
      return lerp(cfg.forwardAngle, cfg.settleAngle - 0.03, (t - 0.32) / 0.68);
    }
    if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) {
      return cfg.settleAngle - 0.05 + Math.sin(now / 130) * 0.018;
    }
    if (gameState === GameState.WAIT_BITE || gameState === GameState.BITE) {
      return cfg.settleAngle - 0.02;
    }
    return cfg.settleAngle;
  }
  function rodPose(now) {
    const w = canvasCssWidth, h = canvasCssHeight;
    const handle = { x: w * 0.93, y: h * 0.865 };
    const displayLength = rodDisplayLength();
    const angle = rodAngle(now);
    const tip = {
      x: handle.x + Math.cos(angle) * displayLength,
      y: handle.y + Math.sin(angle) * displayLength
    };
    return { handle, tip, displayLength, angle, textureKey: rodTextureConfig().key, sections: rodTextureConfig().sections };
  }
  function getWaterY() {
    return canvasCssHeight * currentSpot().waterline;
  }
  function castTarget(distance) {
    const absoluteRatio = clamp(distance / 14.4, 0.12, 1);
    const x = canvasCssWidth * lerp(0.41, 0.71, absoluteRatio);
    const y = lerp(getWaterY() + canvasCssHeight * 0.11, getWaterY() - canvasCssHeight * 0.015, absoluteRatio);
    return { x, y };
  }
  ...
  function drawBackground() {
    ...
    ctx.fillStyle = 'rgba(20,40,35,0.04)';
    ctx.fillRect(0, ch*0.3, cw, ch*0.7);
    ctx.fillStyle = 'rgba(22,68,70,0.05)';
    ctx.fillRect(0, getWaterY(), cw, ch-getWaterY());
  }
  function drawRod(now) {
    const pose = rodPose(now);
    lineAnchor = pose.tip;
    const img = images[pose.textureKey];
    if (!img) return;
    const scale = pose.displayLength / (ROD_SOURCE.tipX - ROD_SOURCE.handleX);
    ctx.save();
    ctx.translate(pose.handle.x, pose.handle.y);
    ctx.rotate(pose.angle);
    ctx.drawImage(
      img,
      -ROD_SOURCE.handleX * scale,
      -ROD_SOURCE.handleY * scale,
      img.width * scale,
      img.height * scale
    );
    ctx.restore();
  }
  function drawFloatSprite(x, y, rotation, scale = 1, hanging = false) {
    const img = images[FLOAT_IMAGE_KEY];
    const drawW = 34 * scale;
    const drawH = 110 * scale;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
    if (img) {
      ctx.drawImage(img, -drawW/2, hanging ? -10*scale : -28*scale, drawW, drawH);
    } else {
  ...
  function drawFloatAndLine(now) {
    ...
    let rigX, rigY, baitX, baitY, hanging = false, rotation = 0;
    if (gameState === GameState.FLIGHT && flightRig) {
      const t = clamp((now - castFlightStart) / castFlightDuration, 0, 1);
      castPoseT = t;
      const ease = 1 - Math.pow(1 - t, 2);
      rigX = lerp(flightRig.from.x, flightRig.to.x, ease);
      rigY = lerp(flightRig.from.y, flightRig.to.y, ease) - Math.sin(Math.PI * t) * (canvasCssHeight * 0.16 + castDistance * 4.2);
      baitX = rigX - 10;
      baitY = rigY + 26;
      const dx = flightRig.to.x - flightRig.from.x;
      const dy = (flightRig.to.y - flightRig.from.y);
      rotation = lerp(-0.55, 0.0, t) + Math.atan2(dy, dx) * 0.06;
      if (t >= 1) {
        floatVisible = true;
        floatLanded = true;
        floatPosition = { ...flightRig.to };
        flightRig = null;
    castPoseT = 0;
        castPoseT = 1;
        clearTimeout(waitBiteTimer);
        waitBiteTimer = setTimeout(() => {
          setState(GameState.WAIT_BITE, '等待鱼口');
          ui['distance-label'].textContent = `${castDistance.toFixed(1)} m`;
          scheduleBite(rnd(2200, 5600));
        }, 520);
      }
    } else if (gameState === GameState.READY || gameState === GameState.CHARGING) {
      hanging = true;
      const hangLen = hangingRigLength();
      const swing = gameState === GameState.CHARGING ? Math.sin(now / 110) * (5 + castPower * 0.06) : Math.sin(now / 440) * 2.4;
      rigX = tip.x - 8 + swing;
      rigY = tip.y + hangLen;
      baitX = rigX + 2;
      baitY = rigY + 52;
      rotation = 0.03 + swing * 0.015;
      floatVisible = true;
    } else if (floatVisible || floatLanded || gameState === GameState.WAIT_BITE || gameState === GameState.BITE || gameState === GameState.FIGHT || gameState === GameState.NET_READY) {
      castPoseT = 0;
      const bobBase = getDepthState().marks === 5 ? -4 : 0;
      let bob = 0;
      if (gameState === GameState.WAIT_BITE) bob = Math.sin(now / 320) * 1.6;
      if (gameState === GameState.BITE) {
        const progress = (performance.now() - biteStartAt) / Math.max(1, biteWindowEnd);
        bob = Math.sin(progress * 10) * 2.2 + (bitePattern === '黑漂' ? 10*progress : bitePattern === '顶漂' ? -6*progress : 4*Math.sin(progress*15));
      }
      if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) bob = Math.sin(now / 120) * 2.8;
      rigX = floatPosition.x;
      rigY = floatPosition.y + bob + bobBase;
      baitX = floatPosition.x + 1.5;
      baitY = floatPosition.y + Math.min(84, 28 + fishingDepth * 18);
      rotation = 0.02;
      floatVisible = true;
    } else {
      return;
    ...
    ctx.lineTo(rigX, rigY - (hanging ? 10 : 24));
    ctx.strokeStyle = 'rgba(240,245,246,0.90)';
    ctx.lineWidth = 1.1;
    ctx.stroke();
    drawFloatSprite(rigX, rigY, rotation, hanging ? 0.92 : 1.0, hanging);
    ctx.beginPath();
    ctx.moveTo(rigX, rigY + 30);
    ctx.lineTo(baitX, baitY - 5);
    ctx.strokeStyle = 'rgba(240,245,246,0.76)';
    ctx.lineWidth = 0.9;
    ctx.stroke();
    drawBaitSprite(baitX, baitY, 1);
    if (!hanging && (gameState === GameState.WAIT_BITE || gameState === GameState.BITE || gameState === GameState.FIGHT || gameState === GameState.NET_READY)) {
      ctx.beginPath();
      ctx.arc(floatPosition.x, floatPosition.y + 24, 18 + Math.sin(now/350)*1.5, 0, Math.PI*2);
  ...
  function castRange(power) {
    const max = maxCastDistance();
    const min = rodLength * 0.55;
  ...
  function startCharging() {
    ...
    castPower = 8;
    chargeDirection = 1;
    setState(GameState.CHARGING, '蓄力抛竿');
  ...
  function releaseCast() {
    ...
    castDistance = +castRange(castPower).toFixed(1);
    floatVisible = false;
    floatLanded = false;
    castPoseT = 0;
    const pose = rodPose(performance.now());
    const target = castTarget(castDistance);
    floatPosition = {...target};
    flightRig = { from: {...pose.tip}, to: target };
    castFlightStart = performance.now();
    castFlightDuration = 900 + castDistance * 36;
    setState(GameState.FLIGHT, '抛投中');
    ui['distance-label'].textContent = `${castDistance.toFixed(1)} m`;
    updateInfoUI();
  }
  ...
  function updateCharging(dt) {
    ...
    if (castPower <= 12) { castPower = 12; chargeDirection = 1; }
    ...
    ui['cast-distance-preview'].textContent = `${castPreviewDistance.toFixed(1)} m`;
    ui['cast-rule-hint'].textContent = `当前${rodLength.toFixed(1)}米竿，最远落点${maxCastDistance().toFixed(1)}米。`;
  }
