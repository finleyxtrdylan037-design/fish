(() => {
  ...
  const VERSION = '0.3.3';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.3.json';
  ...
  const ROD_TEXTURES = {
    '3.6': { key: 'rod:3.6', image: './assets/rods/rod_3_6m.png', heightRatio: 0.40, widthRatio: 1.00, settleAngle: -2.06, backAngle: -1.56, forwardAngle: -2.42 },
    '4.5': { key: 'rod:4.5', image: './assets/rods/rod_4_5m.png', heightRatio: 0.48, widthRatio: 1.13, settleAngle: -2.06, backAngle: -1.55, forwardAngle: -2.43 },
    '6.3': { key: 'rod:6.3', image: './assets/rods/rod_6_3m.png', heightRatio: 0.57, widthRatio: 1.30, settleAngle: -2.04, backAngle: -1.52, forwardAngle: -2.46 },
    '7.2': { key: 'rod:7.2', image: './assets/rods/rod_7_2m.png', heightRatio: 0.63, widthRatio: 1.42, settleAngle: -2.02, backAngle: -1.49, forwardAngle: -2.48 }
  };
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.3 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function rodAngle(now) {
    ...
    if (gameState === GameState.CHARGING) {
      ...
      return lerp(cfg.settleAngle, cfg.backAngle, p);
    }
    if (gameState === GameState.FLIGHT) {
      const t = clamp((now - castFlightStart) / castFlightDuration, 0, 1);
      if (t < 0.38) return lerp(cfg.backAngle, cfg.forwardAngle, t / 0.38);
      return lerp(cfg.forwardAngle, cfg.settleAngle, (t - 0.38) / 0.62);
    }
  ...
  function rodPose(now) {
    ...
    const handle = { x: w * 0.94, y: h * 0.865 };
    ...
    return { handle, tip, displayLength, angle, textureKey: rodTextureConfig().key };
  }
  function getWaterY() {
  ...
  function castTarget(distance) {
    ...
    const x = canvasCssWidth * lerp(0.42, 0.70, absoluteRatio);
    const y = lerp(getWaterY() + canvasCssHeight * 0.105, getWaterY() - canvasCssHeight * 0.012, absoluteRatio);
    return { x, y };
  }
  ...
  function drawBackground() {
    ...
    ctx.fillStyle = 'rgba(20,40,35,0.06)';
    ctx.fillRect(0, ch*0.3, cw, ch*0.7);
    ctx.fillStyle = 'rgba(22,68,70,0.08)';
  ...
  function drawRod(now) {
    ...
    if (!img) return;
    const sourceHandleX = 70;
    const sourceHandleY = 164;
    const sourceTipX = 1995;
    const scale = pose.displayLength / (sourceTipX - sourceHandleX);
    ctx.save();
  ...
  function drawFloatAndLine(now) {
    ...
    if (gameState === GameState.FLIGHT && flightRig) {
      const rawT = clamp((now - castFlightStart) / castFlightDuration, 0, 1);
      const t = clamp((rawT - 0.08) / 0.92, 0, 1);
      const smoothT = 1 - Math.pow(1 - t, 2);
      rigX = lerp(flightRig.from.x, flightRig.to.x, smoothT);
      const arcHeight = Math.min(canvasCssHeight * 0.22, 105 + castDistance * 6.5);
      rigY = lerp(flightRig.from.y, flightRig.to.y, smoothT) - Math.sin(Math.PI*t) * arcHeight;
      if (rawT >= 1) {
        floatVisible = true; floatLanded = true; floatPosition = { ...flightRig.to };
        flightRig = null;
      ...
      hanging = true;
      const swing = gameState === GameState.CHARGING ? Math.sin(now / 90) * (4 + castPower * 0.04) : Math.sin(now / 420) * 2;
      rigX = tip.x - 24 + swing;
      rigY = tip.y + 72;
      ...
      rigX = floatPosition.x; rigY = floatPosition.y + bob + bobBase;
      floatVisible = true;
    } else {
      return;
    }
    ...
    ctx.strokeStyle = 'rgba(240,245,246,0.82)';
    ctx.lineWidth = 1.2;
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(rigX + 3, rigY + 22, 4, 0, Math.PI*2);
    ctx.fillStyle = '#f4d488';
    ctx.fill();
    const marks = ['#ff5858','#ffe172','#1a1a1a','#70d779','#ff5858','#ffe172'];
    const topOffset = hanging ? -18 : (getDepthState().marks === 5 ? -40 : -34);
    marks.forEach((color, i) => {
      ctx.fillStyle = color;
      ctx.fillRect(rigX - 3.5, rigY + topOffset + i*6, 7, 6);
    });
    ctx.beginPath();
    ctx.ellipse(rigX, rigY + (hanging ? 23 : 2), 7.5, 18, 0, 0, Math.PI*2);
    ctx.fillStyle = '#efe6cf';
    ctx.fill();
    ctx.strokeStyle = '#2a3029';
    ctx.lineWidth = 1.4;
    ctx.stroke();
  ...
  function releaseCast() {
    ...
    castDistance = +(rodLength * (0.45 + (castPower / 100) * 1.55)).toFixed(1);
    castDistance = clamp(castDistance, 1.8, maxCastDistance());
    floatVisible = false;
    floatLanded = false;
  ...
  function updateCharging(dt) {
    ...
    castPower += chargeDirection * dt * 0.12;
    if (castPower >= 100) { castPower = 100; chargeDirection = -1; }
    if (castPower <= 18) { castPower = 18; chargeDirection = 1; }
    castPreviewDistance = clamp(rodLength * (0.45 + (castPower / 100) * 1.55), 1.8, maxCastDistance());
  ...
  async function preloadImages() {
    const all = [...SPOTS.map(s => ({ key: s.id, src: s.image })), ...Object.entries(FISH_PROFILES).map(([key,val]) => ({ key: `fish:${key}`, src: val.image })), ...Object.values(ROD_TEXTURES).map(rod => ({ key: rod.key, src: rod.image }))];
    let loaded = 0;
