(() => {
  ...
  function updateFight(dt, now) {
    ...
    tension += ((exhausted ? .45 : hookedFish.profile.power * 2.4) + burst - (exhausted ? 2.1 : 1.5)) * dt;
    fishStamina -= (1.7 + tension * .012) * dt;

    const distanceDelta = (pulse - .54) * hookedFish.profile.power * .55 * dt;
    // 体力归零后鱼不再向外逃跑，只允许保持或被逐步带近。
  ...
  function netFish() {
    ...
    ui["catch-title"].textContent = `${result.species}${rule.directLift ? " · 提鱼上岸" : " · 抄网成功"}`;
