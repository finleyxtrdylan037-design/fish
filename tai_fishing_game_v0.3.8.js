(() => {
  ...
  const VERSION = '0.3.8';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.8.json';
  ...
  const ROD_TEXTURES = {
    '3.6': { key: 'rod:3.6', image: './assets/rods/rod_3_6m.png', sections: 4, heightRatio: 0.24, widthRatio: 0.82 },
    '4.5': { key: 'rod:4.5', image: './assets/rods/rod_4_5m.png', sections: 5, heightRatio: 0.28, widthRatio: 0.92 },
    '6.3': { key: 'rod:6.3', image: './assets/rods/rod_6_3m.png', sections: 6, heightRatio: 0.32, widthRatio: 1.02 },
    '7.2': { key: 'rod:7.2', image: './assets/rods/rod_7_2m.png', sections: 7, heightRatio: 0.36, widthRatio: 1.12 }
  };
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.8 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function heldRigPose(now = performance.now(), geometry = null) {
    ...
    // 浮漂放在靠近钓鱼人的第一节竿附近。
    const refPiece = rod.pieces[0];
    const floatAnchor = lerpPoint(refPiece.start, refPiece.end, 0.78);
    const float = {
      x: floatAnchor.x - 9 - motion.backswing * charge * 0.10 + Math.sin(now / 300) * 0.8,
      y: floatAnchor.y - 7 + charge * 1.6
    };
    // 铅坠、八字环、双子线和饵料位于大把节末端附近，由固定左手虚拟牵住。
    const terminalRoot = {
      x: rod.handle.x + canvasCssWidth * 0.055 - motion.backswing * charge * 0.62,
      y: rod.handle.y - canvasCssHeight * 0.152 + charge * 3 + Math.sin(now / 260) * 0.5
    };
    return { hand, float, terminalRoot };
  }
  function rodAngle(now) {
    ...
    if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) return settle - 0.04 + Math.sin(now / 140) * 0.018;
    if (gameState === GameState.WAIT_BITE || gameState === GameState.BITE) return settle;
    return ready;
  }
  function rodBend(now) {
    ...
    if ((gameState === GameState.FIGHT || gameState === GameState.NET_READY) && hookedFish) {
      const weightFactor = clamp(Math.sqrt(hookedFish.weight / 10), 0.18, 1.45);
      const tensionFactor = clamp(tension / 100, 0, 1);
      const pulse = 0.90 + Math.sin(now / 120 + fishPullPhase) * 0.08;
      return (0.045 + weightFactor * 0.17 + tensionFactor * 0.075) * pulse;
    }
    return 0;
  }
  ...
  function computeRodPieces(now) {
    ...
    // 大把节根部与转动中心保持距离，抛投时根部沿小圆弧移动，而不是钉在一个点上旋转。
    // 控鱼时将持竿位置移到下方控鱼面板上沿偏右，保证整段弯竿可见。
    const fighting = gameState === GameState.FIGHT || gameState === GameState.NET_READY;
    const pivot = fighting
      ? { x: canvasCssWidth * 0.72, y: canvasCssHeight * 0.72 }
  ...
  function castTarget(distance) {
    ...
    // 近投落点稍靠下，远投接近远处水面；横向只在中心左右5%范围内随机。
    const y = lerp(getWaterY() + canvasCssHeight * 0.095, getWaterY() + canvasCssHeight * 0.012, ratio);
    return { x, y };
  }
  ...
  function fightFishVisual(now) {
    if (!hookedFish) return { visible: false, x: floatPosition.x, y: floatPosition.y + 50, scale: 1, angle: 0 };
    ...
    const visible = forceFishSurfaceVisible || gameState === GameState.NET_READY || wave > threshold;
    const lateral = Math.sin(fishPullPhase * 0.72 + fishSurfaceSeed) * canvasCssWidth * (0.10 + weightFactor * 0.045);
    const x = clamp(floatPosition.x + lateral, 34, canvasCssWidth - 34);
    ...
    const y = visible ? surfaceY : deepY;
    const angle = Math.sin(fishPullPhase * 0.55) * 0.18;
    return { visible, x, y, deepY, scale: weightFactor, angle };
  }
  function drawFightFishPartial(visual) {
    ...
    const faceRight = Math.sin(fishPullPhase * 0.72 + fishSurfaceSeed) >= 0;
    ctx.save();
    ...
    ctx.clip();
    ctx.globalAlpha = gameState === GameState.NET_READY ? 0.78 : 0.58;
  ...
  function drawFloatAndLine(now) {
    ...
    if (floatVisible || floatLanded || gameState === GameState.WAIT_BITE || gameState === GameState.BITE || gameState === GameState.FIGHT || gameState === GameState.NET_READY) {
      ...
      if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) bob = Math.sin(now / 120) * 1.8;
      ...
      if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) {
        drawRigLines(tip, fp, null);
        const fishVisual = fightFishVisual(now);
        // 控鱼时显示浮漂到鱼的受力线；等待鱼口时仍不显示水下线组。
        ctx.save();
        ctx.strokeStyle = 'rgba(232,243,241,0.62)';
        ctx.lineWidth = 1.05;
        ctx.beginPath();
        ctx.moveTo(fp.water.x, fp.water.y + 1);
        ctx.quadraticCurveTo((fp.water.x + fishVisual.x) / 2, fp.water.y + 30, fishVisual.x, fishVisual.y - 4);
        ctx.stroke();
        ctx.restore();
  ...
  async function preloadImages() {
    const all = [...SPOTS.map(s => ({ key: s.id, src: s.image })), ...Object.entries(FISH_PROFILES).map(([key,val]) => ({ key: `fish:${key}`, src: val.image })), ...Object.keys(FISH_PROFILES).map(key => ({ key: `fight:${key}`, src: `./assets/fight_fish/${key}.png` })), ...Object.values(ROD_TEXTURES).map(rod => ({ key: rod.key, src: rod.image })), { key: FLOAT_IMAGE_KEY, src: './assets/ui/float_real.png' }, { key: FLOAT_TIP_KEY, src: './assets/ui/float_tip.png' }];
    let loaded = 0;
  ...
  window.__fishGame = {
    getSnapshot: () => ({
      ...
      heldRig: (() => { const h = heldRigPose(performance.now()); return { float: h.float, terminalRoot: h.terminalRoot }; })(),
      ...
      landedRodToFloatRatio: floatVisible && castDistance > 0 ? Math.hypot(floatPosition.x - rodPose(performance.now()).handle.x, floatPosition.y - rodPose(performance.now()).handle.y) / Math.max(1, rodDisplayLength()) : null,
