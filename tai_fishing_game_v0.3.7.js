(() => {
  ...
  const VERSION = '0.3.7';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.7.json';
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.7 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function castTarget(distance) {
    const handle = { x: canvasCssWidth * 0.50, y: canvasCssHeight * 0.945 };
    const absoluteRatio = clamp(distance / 14.4, 0, 1);
    const rodRatio = clamp(distance / Math.max(maxCastDistance(), 0.1), 0, 1);
    // 近区约1.25倍，中区约1.45倍，远区逐步接近1.65倍鱼竿画面长度。
    const desiredRatio = lerp(1.25, 1.65, rodRatio);
    const targetDistance = rodDisplayLength() * desiredRatio;
    let y = lerp(canvasCssHeight * 0.63, canvasCssHeight * 0.437, absoluteRatio);
    let dy = y - handle.y;
    // 当屏幕纵向空间不足时，自动把落点向近处水面收回，但保持距离比例。
    if (Math.abs(dy) > targetDistance * 0.94) {
      y = handle.y - targetDistance * 0.94;
      dy = y - handle.y;
    }
    const dx = Math.sqrt(Math.max(0, targetDistance * targetDistance - dy * dy));
    const x = clamp(handle.x - dx, 28, canvasCssWidth * 0.46);
    return { x, y };
  }
  ...
  async function preloadImages() {
    const all = [...SPOTS.map(s => ({ key: s.id, src: s.image })), ...Object.entries(FISH_PROFILES).map(([key,val]) => ({ key: `fish:${key}`, src: val.image })), ...Object.values(ROD_TEXTURES).map(rod => ({ key: rod.key, src: rod.image })), { key: FLOAT_IMAGE_KEY, src: './assets/ui/float_real.png' }, { key: FLOAT_TIP_KEY, src: './assets/ui/float_tip.png' }];
    let loaded = 0;
