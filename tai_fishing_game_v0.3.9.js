(() => {
  ...
  const VERSION = '0.3.9';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.9.json';
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.9 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function heldRigPose(now = performance.now(), geometry = null) {
    ...
    // 浮漂放在靠近钓鱼人的一节竿附近，并固定在鱼竿画面左侧。
    const oneSectionIndex = clamp(Math.round(rod.pieces.length / rod.sections) - 1, 0, rod.pieces.length - 1);
    const sectionPoint = rod.pieces[oneSectionIndex].end;
    const float = {
      x: sectionPoint.x - 24 - motion.backswing * charge * 0.16 + Math.sin(now / 300) * 0.8,
      y: sectionPoint.y + 8 + charge * 2.0
    };
    // 铅坠、八字环、子线和饵料也位于大把节左侧；左手仅作为固定牵线锚点，不显示。
    const terminalRoot = {
