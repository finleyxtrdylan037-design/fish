(() => {
  ...
  const VERSION = '0.3.2';
  const PROBABILITY_FILE = './bait_depth_fish_probability_v0.3.2.json';
  ...
  const FALLBACK_CONFIG = {
    version: VERSION,
    description: '台钓网页游戏 v0.3.2 出鱼概率配置。每个饵料×钓层组合鱼种概率合计为100%。',
  ...
  function rodPose(now) {
    ...
    const handle = { x: w * 0.915, y: h * 0.845 };
    let tip;
    if (gameState === GameState.CHARGING) {
      const p = castPower / 100;
      tip = { x: lerp(w*0.77, w*0.89, p), y: lerp(h*0.58, h*0.37, p) };
    } else if (gameState === GameState.FLIGHT) {
      const t = clamp((now - castFlightStart) / castFlightDuration, 0, 1);
      const lx = flightRig ? flightRig.to.x : w*0.68;
      tip = { x: lerp(w*0.88, lx + 60, t), y: lerp(h*0.40, h*0.44, t) };
    } else if (gameState === GameState.FIGHT || gameState === GameState.NET_READY) {
      const oscillate = Math.sin(now/120) * 0.012;
      tip = { x: w * (0.765 + oscillate), y: h * (0.465 + Math.cos(now/180)*0.008) };
    } else if (gameState === GameState.WAIT_BITE || gameState === GameState.BITE) {
      tip = { x: w * 0.775, y: h * 0.48 };
    } else {
      tip = { x: w * 0.765, y: h * 0.49 };
    }
    return { handle, tip };
  }
  function getWaterY() {
  ...
  function castTarget(distance) {
    const ratio = clamp(distance / maxCastDistance(), 0.18, 1);
    const x = canvasCssWidth * lerp(0.48, 0.77, ratio);
    const y = getWaterY() + 6;
    return { x, y };
  }
  ...
  function drawRod(now) {
    const { handle, tip } = rodPose(now);
    lineAnchor = tip;
    const control = { x: lerp(handle.x, tip.x, 0.44), y: Math.min(handle.y, tip.y) - 40 };
    const points = [];
    for (let i=0; i<=7; i++) {
      const t = i/7;
      const x = (1-t)*(1-t)*handle.x + 2*(1-t)*t*control.x + t*t*tip.x;
      const y = (1-t)*(1-t)*handle.y + 2*(1-t)*t*control.y + t*t*tip.y;
      points.push({x,y});
    }
    for (let i=0; i<points.length-1; i++) {
      ...
      const width = lerp(13.5, 2.7, i/(points.length-2));
      ctx.lineWidth = width;
      ctx.lineCap = 'round';
      ctx.strokeStyle = i === 0 ? '#342314' : '#313c2f';
      ctx.stroke();
      if (i < points.length-2) {
        ...
        ctx.rotate(angle);
        ctx.strokeStyle = i % 2 === 0 ? '#d94e4e' : '#d5a84a';
        ctx.lineWidth = Math.max(1.2, width * 0.18);
        ctx.beginPath();
        ctx.moveTo(-4, -width*0.35);
        ctx.lineTo(-4, width*0.35);
        ctx.stroke();
        ctx.restore();
      }
    }
    ctx.beginPath();
    ctx.moveTo(handle.x - 28, handle.y + 16);
    ctx.lineTo(handle.x + 2, handle.y - 6);
    ctx.lineWidth = 12;
  ...
  function drawFloatAndLine(now) {
    ...
    if (gameState === GameState.FLIGHT && flightRig) {
      const t = clamp((now - castFlightStart) / castFlightDuration, 0, 1);
      rigX = lerp(flightRig.from.x, flightRig.to.x, t);
      rigY = lerp(flightRig.from.y, flightRig.to.y, t) - Math.sin(Math.PI*t) * Math.min(140, canvasCssHeight * 0.16);
      if (t >= 1) {
    ...
    if (!hanging && (gameState === GameState.WAIT_BITE || gameState === GameState.BITE || gameState === GameState.FIGHT || gameState === GameState.NET_READY)) {
      ctx.beginPath();
      ctx.arc(floatPosition.x, getWaterY() + 28, 18 + Math.sin(now/350)*1.5, 0, Math.PI*2);
  ...
  async function preloadImages() {
    const all = [...SPOTS.map(s => ({ key: s.id, src: s.image })), ...Object.entries(FISH_PROFILES).map(([key,val]) => ({ key: `fish:${key}`, src: val.image }))];
    let loaded = 0;
